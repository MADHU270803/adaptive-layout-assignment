...
- **`src/resolver.ts`** — the engine. Takes one `AdSpec` and one `SurfaceProfile` and produces a `ResolvedElement[]`. Contains all layout decision-making: sizing, direction, priority ordering, degradation. Knows nothing about React, DOM, or CSS.
